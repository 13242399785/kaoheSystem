                ...
                <div class="load"  v-show="parseInt(nowTaskIndex)==1&&!uploadShow">
                    ...
                    <div class="load-title">
                        <div class="load-left" id="selectwapper">
                            <p class="load-input"><span>文件标题</span><el-input v-model="nowData.practiceName" name="filename"  placeholder="请输入标题"></el-input></p>
                        </div>
...
export default {
    ...
    methods:{
        ...
        saveTor(id){
            ...
            // practiceId
            let that=this;
            this.TorpracticeId=id
            console.log(this.TorpracticeId)
            for(let i=0;i<this.gradeList.length;i++){
                this.gradeList[i].practiceId=this.TorpracticeId
                for(let j=0;j<this.gradeList[i].children.length;j++){
                    this.gradeList[i].children[j].practiceId=this.TorpracticeId
                }
            }
            console.log(this.gradeList)
            this.$api.Training.addQuestions(this.gradeList).then(res=>{
               if(res.data.success){
        ...
        getFile(nowitem){ 
            ...
            let params={
                "practiceId": 0,
                "practiceName": this.nowData.practiceName,
            ...
            if(this.nowTaskIndex==0){//当是任务书时
                params.practiceName=this.torName
            ...
            this.$api.Training.addTeaching(params).then(res=>{
                if(res.data.success){
                    if(nowitem){//当保存任务书时候 得到任务书id同时保存试题内容
                        let id=res.data.result.practiceId
                        that.saveTor(id)
                    }else{
