            ...
            <div class="dialog-wapper">
                <div class="dialog-list">
                   <p class="flex-lr">
                        ...
                        <el-input v-model="setData.competitionName" placeholder="请输入标题名称"></el-input>
                   </p>
                   <p class="flex-lr">
        ...
        <el-dialog
            ...
            <div class="dialog-wapper">
                ...
                <div class="tortitle flex-between">
                    <span>任务书标题：</span>
                    <el-input v-model="nowTor.competitionName"></el-input>
                </div>
                <div class="addfen">
    ...
    methods:{
        ...
        handleSelectionChange(val) {
            ...
            for(let i=0;i<val.length;i++){
                this.chooseList.push(val[i].competitionId)
            }    
        ...
        alldelSave(){
            let that=this;
            this.$api.Competition.chooseDelTeaching(this.chooseList).then(res=>{
                if(res.data.success){
        ...
        setEm(item){
            ...
            if(item.classId===4){//任务书
                ...
                this.nowTor.competitionName=item.competitionName;
                this.TorcompetitionId=item.competitionId
                this.pdfUrl=this.$api.serverUrl+'/'+ encodeURI(this.nowTor.url)
                // console.log(this.pdfUrl)
                this.getTorlist(item.competitionId)
            }else{
        ...
        treSave(){
            ...
            this.saveRole()
            this.$api.Competition.updateQuestions(this.gradeList).then(res=>{
               console.log(res.data)
        ...
        getTorlist(id){
            ...
            this.$api.Competition.getTestQuestionsTreeList(params).then(res=>{
               console.log(res.data)
               that.gradeList=res.data.result
            }).catch((error) => {
                console.error(error);
        ...
        removeScore(id){
            let that=this;
            this.$api.Competition.deleteTestQuestions(id).then(res=>{
        ...
        saveRole(){
            let that=this;
            if(this.setData.competitionName==''){
            ...
            if(this.setData.classId==4){//任务书
                ...
                this.setData.competitionName=this.nowTor.competitionName;
            }
            this.$api.Competition.edirtTeaching(this.setData).then(res=>{
                if(res.data.success){
        ...
        delSave(){
            ...
            //     return false;
            // }
            this.$api.Competition.delTeaching(this.competitionId).then(res=>{
                if(res.data.success){
                    that.$message.success('删除成功！')
        ...
        getList(){
            let that=this;
            let params={
                "competitionName":this.nowData.nowName,
                "subjectId":this.nowData.subjectId,
                "classId":this.nowData.classId,
                "pageSize":this.pagination.size,
                "pageIndex":this.pagination.currentPage
            }
            this.$api.Competition.getTeachingAll(params).then(res=>{
        ...
        showPlay(item){
            ...
            console.log(type)
            if(type=='.mp4'){
                this.$router.push({name:'teacherCompetitionPlay',params:{id:item.competitionId}})	
            }else{
                this.$router.push({name:'teacherCompetition',params:{id:item.competitionId}})	
            }
        }
    }
}
